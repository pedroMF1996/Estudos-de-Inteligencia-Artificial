# -*- coding: utf-8 -*-
"""
Created on Wed Oct 30 22:50:09 2019
...
def pesqisa_randomica(dominio, funcao_custo):
...
#solucao_subidaEncosta = subida_encosta(dominio, funcao_custo)
#custo_subida_encosta = funcao_custo(solucao_subidaEncosta)
#imprimir_agenda(solucao_subidaEncosta)

def Tempera_Simulada(dominio, funcao_custo, temperatura = 99999999.99, resfriamento = 0.95, passo = 1):
    solucao = [random.randint(dominio[i][0],dominio[i][1]) for i in range(len(dominio))]
    ...
    while temperatura > 0.1:
        ...
        elif solucao_temp[i] > dominio[i][0]:
...
s3 = crossover(dominio, s1, s2)

def genetico(dominio, funcao_custo, tamanho_populacao = 500, passo = 1, 
             probabilidade_mutacao = 0.2, elitismo = 0.15, numero_geracoes = 1000):
    populacao = []
        ...
        while len(populacao) < tamanho_populacao:
            if random.random() < probabilidade_mutacao:
                ...
                while c1 == c2:
                    c1 = random.randint(0, numero_elitismo)
                    c2 = random.randint(0, numero_elitismo)
                populacao.append(crossover(dominio, individuos_ordenados[c1], individuos_ordenados[c2]))
    return custos[0][1]
